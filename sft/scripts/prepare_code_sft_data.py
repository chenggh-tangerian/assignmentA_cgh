#!/usr/bin/env python3
"""
功能: 将 python-code Alpaca parquet 转为 LLaMA-Factory SFT 用的 instruction/input/output JSON。

输入:
  --source_dir   原始数据集目录 (默认 python_code_instructions_18k_alpaca)
...
def read_parquet(path: Path) -> list[dict[str, Any]]:
    """读取单个 parquet 文件为行列表（pandas / pyarrow / datasets 兜底）。"""
    try:
        import pandas as pd
...
def find_parquet_files(source_dir: Path) -> list[Path]:
    """在 source_dir 或其 data/ 子目录下查找全部 parquet。"""
...
def deduplicate(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """按 (instruction, input, output) 去重，保留首次出现。"""
...
def main() -> None:
    """读 parquet → 清洗去重 → 划分 → 写出 SFT 数据集与注册表。"""
