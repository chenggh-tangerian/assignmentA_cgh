#!/usr/bin/env bash
# CoT 基线评测：结构化 Reasoning / Key steps / Final code，temperature=0 解码。
#
# 输入
#   - 环境变量（可选）:
#       MODEL_PATH          模型路径（默认 rl/outputs/train_lora_ppo_full/checkpoint-1304）
#       OUTPUT_DIR          输出目录（默认 a5_cgh/outputs/eval_cot）
#       MBPP_CONFIG         sanitized|full（默认 sanitized）
#       MBPP_SPLIT          默认 test
#       LIMIT / BATCH_SIZE / MAX_NEW_TOKENS / GPU_ID
#       COT_INCLUDE_EXAMPLES  非空则开启 few-shot CoT 示例
...
#   LIMIT=20 MODEL_PATH=rl/outputs/train_lora_ppo_full/checkpoint-1304 bash a5_cgh/scripts/eval_cot.sh
#   COT_INCLUDE_EXAMPLES=1 bash a5_cgh/scripts/eval_cot.sh
...
if command -v nvidia-smi >/dev/null 2>&1; then
  echo "[eval_cot] GPU status before launch:"
  nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader || true
  echo "[eval_cot] Tip: do not run other evals or training on the same GPU concurrently."
fi
...
if [[ -n "${COT_INCLUDE_EXAMPLES:-}" ]]; then
  EXTRA_ARGS+=(--cot_include_examples)
fi
...
"${PYTHON_BIN}" a5_cgh/mbpp_eval_baseline.py \
  ...
  --output_dir "${OUTPUT_DIR:-a5_cgh/outputs/eval_cot}" \
