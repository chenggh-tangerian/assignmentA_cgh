"""CoT Prompt 模板与 Final code 抽取（与 tts/cot_code_example.py 同名对齐）。

输入 / 输出（函数契约）
------------------------
build_prompt(task_description, include_examples=False) -> str
    入：题目文本（可含 'Your code should pass these tests:' + asserts）
    出：user 侧 CoT 前缀（Task: ... + Reasoning:），可选 few-shot 示例

build_chat_messages(task_description, include_examples=False) -> list[dict]
    入：同上
    出：[{role:system, content}, {role:user, content}]，供 chat_template 使用

extract_final_code(response) -> str
    入：模型完整 CoT 回复
    出：Final code 代码块文本（评测只认这一段）

normalize_text(text) -> str
    入：任意文本；出：去首尾空行、行尾空白后的规范化字符串

本文件无文件 I/O，由 mbpp_eval_baseline.py --method cot 调用。
"""
...
def build_prompt(task_description: str, include_examples: bool = False) -> str:
    """填充 Task 文本；可选拼接 few-shot CoT 示例后返回 user 前缀。"""
...
def build_chat_messages(task_description: str, include_examples: bool = False) -> list[dict[str, str]]:
    """组装 system + user 的 CoT chat messages（供 tokenizer chat_template 使用）。"""
    return [
...
def extract_final_code(response: object) -> str:
    """从 CoT 回复中抽取 Final code 代码块（评测只用这一段）。"""
